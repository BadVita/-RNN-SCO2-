...
optimizer = torch.optim.Adam(model.parameters(),lr=learning_rate,weight_decay=4e-6)
...
# 预测结果并绘制图像
with torch.no_grad():
    pred = scaler.inverse_transform(model(x).numpy())
...
torch.save(model.state_dict(), 'D:/dl/data/model/k.pth')
relative_error = np.abs(actual - pred) / actual
